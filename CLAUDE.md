...
- Summary는 육하원칙을 지킨다. 다음 6가지가 본문에서 모두 답이 되어야 한다.
  - 누가: 이 변경을 요청하거나 트리거한 주체. 사용자 지시, 로드맵의 어느 Phase, 어떤 이슈 등.
  - 언제: 어느 Phase 또는 어느 시점(예: Phase 2 완료 후 Phase 3 진입 시점)에 왜 지금 필요한지.
  - 어디서: 변경이 닿은 레이어, 디렉토리, 파일. 구조 설명은 6절의 의존성 방향에 맞춰 서술.
  - 무엇을: 실제로 추가, 삭제, 수정된 결과물. 결과물 자체를 명시하고 두루뭉술한 요약은 금지.
  - 어떻게: 채택한 접근법과 대안, 라자한 선택을 한 근거, 남긴 트레이드오프.
  - 왜: 이 변경이 없으면 무엇이 막히는지, 어떤 문제나 결함을 해결하는지.
